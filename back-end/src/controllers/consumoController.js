...
const getAllCosumptions = async (req, res) => {
    try {
        const consumptions = await Consumo.findAll();
...
const getConsumptionById = async (req, res) => {
    try {
        const consumption = await Consumo.findByPk(req.params.idconsumo);
        if (consumption) {
            res.json(consumption);
        } else {
            res.status(404).json({ message: 'El consumo ingresado no existe' });
        }
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
