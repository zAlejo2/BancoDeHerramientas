...
const ImportarExcel = () => {
    ...
    const handleExcelUpload = async () => {
        ...
        try {
            ...
            const mensaje = error.response?.data?.errors?.map(err => err.errors.join(' --- ')).join(' --- ') || "Error al procesar el archivo, asegúrate que sea excel (.xlsx) y no esté corrupto. Recuerda que si hiciste cambios en el archivo debes recargar la página e ingreasar el excel de nuevo. Revisa tu conexión a internet y si aún no funciona intenta recargar la página o inicia sesión de nuevo";
